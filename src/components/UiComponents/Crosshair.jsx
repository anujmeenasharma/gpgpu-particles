...
const Crosshair = ({ color = 'white', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    // Scroll trigger to hide crosshair on scroll and show it again when scroll stops
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        if (self.direction !== 0) {
          // Scrolling - hide crosshair
          gsap.to(cursorRef.current, { opacity: 0, duration: 0.3 });
        }
      }
    });

    // Show crosshair again when scroll stops
    let scrollTimeout;
    const handleScroll = () => {
      clearTimeout(scrollTimeout);
      scrollTimeout = setTimeout(() => {
        gsap.to(cursorRef.current, { opacity: 0.35, duration: 0.3 });
      }, 150);
    };
